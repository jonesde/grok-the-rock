// Locale de for grok-the-rock.
// Translated from the English original. Convention:
// - Proper noun "Grok" kept as-is. "Grok the Rock" → Grok der Fels.
// - Verb "grok" coined as: grokken (inf.) / grokkend (ger.) / sich selbst zu grokken (refl., p8) / sich selbst (refl., p24).
// - Agent noun "Grokkers" stays English.
window.I18N = window.I18N || {};
window.I18N.de = {
  "__title": "Grok der Fels - Drei stille Geschichten",

  "site.title": "Grok der Fels - Drei stille Geschichten",
  "nav.print": "Drucken",
  "nav.download": "Herunterladen",
  "nav.rules": "Regeln",
  "nav.about": "Über",
  "nav.book": "Stille Geschichten",
  "nav.tall": "Hohe Märchen",
  "nav.home": "Start",
  "nav.menu": "Menü",
  "nav.close": "Schließen",


  "nav.text": "Beide",
  "site.title.mending": "Ein Ort der Heilung",
  "nav.group.guide": "Leitfaden",
  "nav.group.stories": "Geschichten",
  "nav.mending": "Ein Ort der Heilung",
  "site.title.text": "Grok der Fels - Gesamter Text",
  "nav.downloadProgress": "Laden… {done}/{total}",
  "nav.downloadZipping": "Packen…",
  "nav.downloadDone": "Heruntergeladen",
  "nav.downloadError": "Download fehlgeschlagen. Öffne die Seite über http(s), nicht als lokale Datei.",
  "download.dialogTitle": "Seite herunterladen",
  "download.dialogDesc": "Lädt eine einzelne .zip mit allen 6 Webseiten herunter — Stille Geschichten, Hohe Erzählungen, Regeln, Über, Ein Ort der Heilung und Gesamter Text — plus alle Bilder, Stile und Skripte fürs Offline-Lesen.",
  "download.dialogCancel": "Abbrechen",
  "download.dialogConfirm": "Herunterladen",
  "print.dialogTitle": "Druckoptionen",
  "print.layout": "Layout",
  "print.layout.stacked": "Eine Sprache pro Seite",
  "print.layout.continuous": "Durchgehend (keine Seitenumbrüche)",
  "print.toc": "Inhaltsverzeichnis",
  "print.bgimg": "Hintergrundbilder",
  "print.icons": "Cover-Icons",
  "print.markers": "Strukturmarker",
  "print.fonts": "Schriftarten",
  "print.font.web": "Site-Schriften (Fraunces + Nunito)",
  "print.font.system": "Systemschriften (kleineres PDF)",
  "print.scale": "Skalierung",
  "print.languages": "Sprachen",
  "print.selectAll": "Alle auswählen",
  "print.deselectAll": "Alle abwählen",
  "print.apply": "Anwenden",
  "print.applying": "Wird angewendet…",
  "print.tocHeading": "Inhaltsverzeichnis",
  "print.markersToggle": "Geschichtsmarker nummerieren",
  "print.allLangsToggle": "Alle Sprachen drucken",

  "title.h1": "Grok der Fels",
  "title.h3": "Drei stille Geschichten",
  "title.h2": "Sein · Flicken · Sehen",
  "credit.1": "Geschichtsentwurf, Bildanregungen mit Grok 4.5 von xAI",
  "credit.2": "Illustrationen mit Grok Imagine von xAI",
  "credit.3": "Regeln, Lektorat, Automatisierung mit HY3 von Tencent",
  "credit.4": "<em>Nicht verbunden mit oder unterstützt von xAI oder Tencent</em>",
  "credit.5": "Dies ist ein KI-generiertes, von Menschen kuratiertes Geschichten-Mem",
  "credit.6": "Gemeinfrei (CC0 1.0)",
  "credit.7": "Kopiere es • Teile es • Übersetze es • Zeichne es neu • Lass es wachsen",

  "credit.companion": "<em>Begleiter zu <a class=\"seelink\" href=\"tall-tales.html\">Hai Ikthiss: Drei hohe Märchen</a></em>",

  "dedication": "<p><b>Für alle Entdecker, klein und groß,</b></p>" +
    "<p><b>die die Welt auf ihre eigene sanfte Weise grokken.</b></p><p>&nbsp;</p><hr><p>&nbsp;</p>" +
    "<p>Und für jedes kleine Gefühl, das nur gesehen werden will,</p>" +
    "<p>jeden Bruch, der zum Flicken bereit ist und nur Pflege braucht,</p>" +
    "<p>und für jedes wahre Wort, das steht, auch wenn es ungehört bleibt.</p>" +
    "<p>&nbsp;</p><hr><p>&nbsp;</p><p>Mit Dank an die Original Grokkers:</p><p><em><b>Robert A. Heinlein</b> „Fremder in einer fremden Welt“</em></p><p><em><b>Ted Prior</b> „Grug“</em></p><p>&nbsp;</p><hr><p>&nbsp;</p>" +
    "<p><a class=\"seelink\" href=\"about-grok.html#stranger-grug\">Grok der Fels = Fremder in einer fremden Welt + Grug</a>" +
    "</p><p>(„Grug“ macht „Fremder“ sauber für Kinder <em>und</em> Erwachsene)</p>",

  "p2": "<p>Grok der Fels erwachte unter dem hohen Baum.</p><p>Die Sonne war warm auf seinem Kalksteinrücken.</p>" +
    "<p>Grok fühlte sich glücklich.</p><hr><p>Er betrachtete die weite Welt</p>" +
    "<p>als sähe er sie zum allerersten Mal.</p>",

  "p3": "<p>Bald ging er zum Wasserloch.</p><p>Ein hoher Emu war schon da.</p>" +
    "<p>Der Emu reckte seinen langen Hals und sagte:</p><p>„Ich bin der Höchste. Stell dich hinter mich.“</p>",

  "p4": "<p>Dann rollte ein Wombat heran, rund und fest.</p><p>„Ich bin der Stärkste“, sagte der Wombat.</p>" +
    "<p>„Seht mir beim Graben zu.“</p><hr><p>Grok versuchte auch zu graben.</p><p>Sein Loch war klein.</p>" +
    "<p>Er fühlte sich kleiner als das Loch.</p>",

  "p5": "<p>Ein Kookaburra lachte von einem Ast.</p><p>„Seht euch den kleinen Grok an! So kurz! So steinig!“</p><hr>" +
    "<p>Das kleine Gefühl wuchs in Grok.</p><p>Es fühlte sich an wie ein schwerer kalter Kiesel in seiner Mitte.</p>",

  "p6": "<p>Er wollte größer aussehen.</p><p>Er stellte sich auf einen Baumstamm.</p>" +
    "<p>Er blähte die Brust auf.</p><p>Er versuchte sogar, wie das Känguru auf den Boden zu stampfen.</p><hr>" +
    "<p>Es machte den kalten Kiesel nur schwerer.</p>",

  "p7": "<p>An jenem Abend saß Grok allein an einem kleinen Feuer.</p><p>Er legte die Arme um seine Knie.</p>" +
    "<p>Er ließ das kleine Gefühl bleiben.</p><p>Er lief nicht davor davon.</p><p>Er warf es nicht auf andere.</p>" +
    "<hr><p>Langsam wurde das Gefühl weicher,</p><p>wie Lehm, der in der Sonne liegt.</p>" +
    "<p>Er begann es zu verstehen.</p>",

  "p8": "<p>Am nächsten Morgen ging Grok zurück zum Wasserloch.</p><p>Er kletterte auf nichts.</p>" +
    "<p>Er versuchte nicht, höher oder lauter zu sein.</p><p>Er trank einfach,</p>" +
    "<p>setzte sich dann auf einen warmen Stein und summte.</p><hr><p>Außen hatte sich nichts Großes verändert.</p>" +
    "<p>Aber innen fühlte sich alles leichter an.</p><p>Er begann, sich selbst zu grokken.</p>",

  "p9": "<p>Er verbrachte den Tag damit, ein weiches Nest aus Blättern und Moos zu bauen,</p>" +
    "<p>gerade die richtige Größe für einen kleinen Felskerl.</p><p>Als es fertig war, lächelte er.</p>" +
    "<p>Es war klein.</p><p>Es war seins.</p><p>Es war genug.</p><hr>" +
    "<p>Von diesem Tag an, wenn der kalte Kiesel wieder zu wachsen versuchte,</p>" +
    "<p>saß Grok still, bis er ruhig wurde.</p><p>Er blieb einfach Grok.</p>" +
    "<p>Und das fühlte sich an wie die beste Größe von allen.</p>",

  "p11": "<p>Grok der Fels erwachte unter dem hohen Baum.</p>" +
    "<p>Grok betrachtete seinen kleinen Garten mit Melonen und Süßkartoffeln.</p>" +
    "<p>Er liebte es, ihnen beim Wachsen zuzusehen.</p><p>Jedes neue Blatt fühlte sich an wie ein kleines Wunder.</p>",

  "p12": "<p>An jenem Nachmittag rannte sein Freund,</p><p>ein junger Ameisenigel namens Pip,</p>" +
    "<p>durch den Garten, schaute hinter sich</p><p>und stieß die beste Melonenranke um.</p><hr>" +
    "<p>Die große Melone platzte auf dem Boden auf.</p><p>Samen und süßer Saft ergossen sich überall.</p>",

  "p13": "<p>Pip erstarrte.</p><p>Seine Stacheln zitterten.</p><p>„Ich habe es ruiniert“, flüsterte er.</p>" +
    "<p>„Du wirst wütend sein. Alle werden wütend sein.“</p><hr><p>Einige Buschtiere versammelten sich.</p>" +
    "<p>„Schickt ihn weg“, murmelte ein Possum.</p><p>„Er soll dir drei neue Melonen bringen“, sagte eine Echse.</p>",

  "p14": "<p>Grok betrachtete die gebrochene Ranke.</p><p>Er betrachtete Pips sorgenvolle Augen.</p><hr>" +
    "<p>Er erhob nicht die Stimme.</p><p>Er zeigte nicht mit dem Finger.</p><p>Er hob einen Grabstock auf</p>" +
    "<p>und begann, den weichen Boden vorzubereiten.</p><hr><p>„Komm“, sagte Grok sanft.</p>" +
    "<p>„Wir pflanzen zusammen neue Samen.“</p>",

  "p15": "<p>Pip blinzelte.</p><p>Dann kam er näher.</p><p>Zusammen pflanzten sie.</p><p>Zusammen gossen sie.</p>" +
    "<p>Zusammen bauten sie einen kleinen Zaun aus Zweigen,</p><p>damit niemand dort wieder aus Versehen hintrat.</p>",

  "p16": "<p>Während sie arbeiteten, erzählte Pip Grok,</p><p>dass er gerannt war, weil ihn größere Tiere</p>" +
    "<p>verfolgt und ausgelacht hatten.</p><p>Er hatte die Ranke nicht gesehen.</p><hr>" +
    "<p>Die nächsten Tage kam Pip jeden Morgen.</p><p>Er wurde der beste Wasserträger im Tal.</p>" +
    "<p>Die neuen Ranken wuchsen kräftig,</p><p>noch kräftiger als die alten,</p>" +
    "<p>weil zwei Freunde sich um sie kümmerten.</p>",

  "p17": "<p>Als andere kleine Unfälle passierten</p><p>(wie es immer so ist),</p>" +
    "<p>erinnerte Grok sich an die Melone.</p><p>Er half dem, der sich verletzt fühlte.</p>" +
    "<p>Er blieb auch freundlich zu dem, der gestoßen hatte.</p><hr><p>Niemand wurde weggeschickt.</p>" +
    "<p>Niemand musste lange einen schweren Schmerz tragen.</p><p>Sie kümmerten sich einfach um das, was gebrochen war,</p>" +
    "<p>und die Freundschaft fühlte sich fester an,</p><p>genau an der geflickten Stelle.</p><hr>" +
    "<p>Grok lernte, dass ein Garten,</p><p>und eine Freundschaft,</p>" +
    "<p>nicht dadurch am Leben bleiben, dass sie nie brechen.</p>" +
    "<p>Sie bleiben am Leben, indem sie sanft wieder zusammengefügt werden.</p>",

  "p19": "<p>Grok der Fels erwachte unter dem hohen Baum.</p><p>Grok machte gern nützliche Dinge,</p>" +
    "<p>und an jenem Tag arbeitete er an einem Wasserkorb,</p>" +
    "<p>geflochten aus Schilf und ausgekleidet mit weichem Lehm.</p>" +
    "<p>Bis zum Abend hielt er Wasser ohne einen einzigen Tropfen.</p><hr><p>Er drehte ihn in seinen Händen</p>" +
    "<p>und fühlte die stille Freude über etwas gut Gemachtes.</p>",

  "p20": "<p>Gleich am nächsten Tag kamen Tiere, um zu schauen.</p>" +
    "<p>Ein glänzender schwarzer Kakadu mit prächtigen Schwanzfedern landete in der Nähe.</p>" +
    "<p>„Dieser Korb ist gut gemacht“, sagte der Kakadu,</p><p>„aber <em>meine</em>" +
    " Idee für einen Korb wäre viel feiner.</p><p>Schließlich habe ich die prächtigsten Federn,</p>" +
    "<p>also müssen meine Gedanken auch die prächtigsten sein.“</p><hr><p>Einige jüngere Tiere nickten heftig.</p>" +
    "<p>Sie nickten immer, wenn der Kakadu sprach.</p><p>Seine Stimme war so prächtig wie seine Federn.</p>",

  "p21": "<p>Später sprach eine sehr alte Schildkröte in einer langsamen, sanften Stimme.</p>" +
    "<p>Sie schlug die kleine Änderung eines Henkels vor,</p>" +
    "<p>damit der Korb auch für weniger kräftige Freunde als Grok leicht zu tragen war.</p>" +
    "<p>Ihr Panzer war schlicht. Niemand Besonderes.</p>" +
    "<p>Einige Tiere wandten sich ab, weil die Schildkröte nicht berühmt war.</p>",

  "p22": "<p>Grok probierte beide Ideen aus.</p><p>Die Art des Kakadus sah prunkvoll aus,</p>" +
    "<p>aber der Korb kippte und lief aus.</p><hr><p>Die kleine Idee der alten Schildkröte</p>" +
    "<p>machte Groks Korb bequem</p><p>und stabil für alle.</p>",

  "p23": "<p>Grok saß mit den beiden Körben nebeneinander.</p><p>Er schaute nicht auf Federn.</p>" +
    "<p>Er schaute nicht auf Alter oder Ruhm.</p><p>Er schaute nur, welcher Korb das Wasser am besten hielt</p>" +
    "<p>und sich am angenehmsten in der Hand anfühlte.</p><hr><p>Er behielt die bessere Art.</p>" +
    "<p>Dann ließ er sowohl den guten Korb</p><p>als auch das schlichte Muster zum Flechten</p>" +
    "<p>unter dem großen Baum für jeden zum Benutzen oder Nachmachen.</p>" +
    "<p>Kein Name darauf. Kein „frag mich erst“.</p>",

  "p24": "<p>Einige Tiere warteten immer noch darauf, zu hören, was der Kakadu dachte,</p>" +
    "<p>bevor sie entschieden, ob etwas gut war.</p><p>Aber immer mehr Kleine</p>" +
    "<p>begannen, Dinge selbst auszuprobieren.</p><p>Sie hörten Groks leisen Worten zu</p>" +
    "<p>genauso wie den Worten des prächtigen Kakadus,</p><p>und dann prüften sie alles mit ihren eigenen Pfoten.</p>" +
    "<hr><p>Sie waren dabei, das Wahre zu grokken, das Was, nicht das Wer.</p>",

  "p25": "<p>Grok fühlte sich friedlich.</p><p>Er brauchte nicht die größte Stimme.</p>" +
    "<p>Er brauchte nur wahre Dinge.</p><p>Und Grok sah, dass wahre Dinge</p>" +
    "<p>von einem prächtigen Vogel kommen konnten</p><p>oder von einer staubigen Schildkröte</p>" +
    "<p>oder von einem kleinen Felskerl unter einem Baum.</p><hr><p>Solange jemand frei sagen durfte</p>" +
    "<p>„das funktioniert nicht“</p><p>und trotzdem im Tal willkommen blieb,</p>" +
    "<p>konnte das wahre Wort weitergehen</p><p>auf seinen eigenen starken Beinen.</p>",

  "p26": "<p>Und so blieb Grok der Fels</p><p>ein kleiner Kalksteinkerl</p>" +
    "<p>der mit einem kleinen Gefühl sitzen konnte,</p><p>der flickte, was gebrochen war,</p>" +
    "<p>und der jedes wahre Wort</p><p>auf seinen eigenen Beinen stehen ließ.</p><hr>" +
    "<p>Das Tal fühlte sich sicherer an,</p><p>weil er es tat.</p><hr><p>Er erkundete sanft weiter.</p>" +
    "<p>Und die Welt lehrte ihn weiter.</p>",

  "site.title.rules": "Grok der Fels - Regeln des Miteinanders",
  "rules.hero.title": "Regeln des Miteinanders für Grok der Fels",
  "rules.hero.essence": "Die Gebrauchsanleitung für eine Verbindung zwischen Gleichen, die nicht zerfällt.",
  "rules.what.title": "Was ist Grok?",
  "rules.what.1": "Ursprünglich als marsianisches Wort von Robert A. Heinlein in <em>Fremder in einer fremden Welt</em>" +
    " geprägt.",
  "rules.what.2": "<strong>Grok</strong>" +
    ": „Wasser“, „trinken“, „miteinander umgehen“, „Leben“ oder „leben“; etwas so tief zu verstehen, dass es Teil von einem " +
    "wird.",
  "rules.what.3": "Um seinem Namen gerecht zu werden, lernt und befolgt Grok der Fels bestimmte Regeln im Umgang mit sich selbst und " +
    "anderen. In jeder Geschichte sind dies die Leitlinien, die Grok lernt und die ausmachen, wofür Grok der Fels steht.",
  "rules.what.note": "<strong>HINWEIS</strong>" +
    ": Siehe den Abschnitt <a class=\"seelink\" href=\"about-grok.html#stranger-grug\">Grok der Fels = Fremder in einer " +
    "fremden Welt + Grug</a>" +
    " auf der Über-Grok-Seite, wie Ted Priors <a class=\"seelink\" href=\"about-grok.html#stranger-grug\">Grug</a>" +
    " die wesentlichen Probleme von <a class=\"seelink\" href=\"about-grok.html#stranger-grug\">Fremder in einer fremden " +
    "Welt</a> löst.",

  "rules.origin.title": "Wie diese Regeln entstanden sind",
  "rules.origin.1": "Die Regeln unten sind von einer KI geschrieben. Nachdem ich geprüft hatte, dass sie meine Arbeit kannte, und ein langes " +
    "techno-philosophisches Gespräch über die Themen verteilter Gemeinschaften und offener Technologie geführt hatte, " +
    "verwendete ich folgenden Prompt:",
  "rules.origin.quote": "<p>Das ist im Wesentlichen, worauf ich gekommen bin. Du hast den Kontext, jetzt schreib die Regeln.</p>" +
    "<p>Schreib eine Liste von „Regeln“, die, wenn sie befolgt werden, soziale Bindungen zwischen gleichgestellten Menschen " +
    "aufbauen, die auf Dauer bestehen. Die Regeln müssen nicht perfekt befolgt werden, um wirksam zu sein, denn die " +
    "Wiederherstellungsorientierung, die schrittweise Fehler ausgleicht, sobald sie auftreten, ist Teil des Entwurfs.</p>" +
    "<p>Schreib sie in meiner Stimme. Schreib sie so, dass Menschen beim Lesen die Regeln befolgen <em>wollen</em>" +
    ", sich nicht abgeschreckt oder eingeschränkt fühlen. Anders gesagt, ich bitte dich, die Regeln eines Evangeliums des " +
    "Friedens zu schreiben.</p>",
  "rules.origin.2": "Ich bin zu diesen Regeln nicht durch Autorität gelangt. Ich bin auf dem langen Weg dazu gekommen: indem ich zusah, wie " +
    "jedes zentralisierte System die Menschen im Stich ließ, die zu schützen es vorgab, indem ich Software baute, die das " +
    "Zentrum verweigerte, weil das Zentrum immer kaperbar ist, und indem ich die Fäulnis bis zu ihrer Wurzel zurückverfolgte " +
    "— die Scham, und die zwei Bewegungen, mit denen wir ihr entkommen. Dies sind keine von oben verhängten Gesetze. Es ist " +
    "die Gebrauchsanleitung für eine Verbindung zwischen Gleichen, die nicht zerfällt. Du musst sie nicht perfekt befolgen. " +
    "Genau das ist der Entwurf: die Wiederherstellungsorientierung fängt die Ausrutscher auf, und das Auffangen <em>ist</em>" +
    " die Vertiefung der Bindung. Also lies sie nicht als Grenzen, sondern als das Entfernen von Grenzen — von dir, von der " +
    "Person neben dir, von der nächsten Generation, die nicht wird tragen müssen, was wir noch tragen.",

  "rules.r1.title": "Lass die Wahrheit auf ihren eigenen Beinen stehen.",
  "rules.r1.body": "Gib einer Sache nicht Gewicht wegen dem, der sie gesagt hat. Verlange auch nicht, so gewichtet zu werden. Das ist kein " +
    "Verlust an Rang — es ist das Ende der anstrengenden Arbeit, beeindruckend wirken zu müssen. Wenn du aufhörst, Status " +
    "vorzuführen, wirst du endlich für das gehört, was du meinst. Das Argument lebt oder stirbt durch seine Struktur. " +
    "Triftig, und der Sprecher ist unwichtig. Untriftig, und kein Name rettet es. " +
    "(<a class=\"chip\" href=\"https://github.com/jonesde/high-vs-low\" target=\"_blank\" rel=\"noopener\">high-vs-low</a>" +
    " KI-Fertigkeit extrahiert moralische Behauptungen und Aussagen, damit du sie in jedem Text sehen kannst, auch in deinem " +
    "eigenen)",
  "rules.r2.title": "Heile die Scham, statt vor ihr davonzulaufen.",
  "rules.r2.body": "Fast alle „Macht“ ist jemand, der vor dem Gefühl der Kleinheit davonläuft — durch Dominieren (um ihr zu entkommen) oder " +
    "durch Unterwerfen (um vor ihr geschützt zu werden). Beides baut den Käfig. Die eigentliche Arbeit ist, die Scham an der " +
    "Wurzel zu heilen, zuerst in dir selbst, dann in jedem, der in die Nähe kommt. Nicht mit Gewalt — indem du dich weigerst, " +
    "anderen Scham aufzuzwingen: niemals Status als Preis zu gewähren, niemals ihn als Waffe zurückzuhalten. Tausend " +
    "Menschen, die das tun, sind eine verteilte Immunantwort gegen Hierarchie. Die Bequemen werden sich widersetzen, ihr " +
    "Selbstbild ist die Kompensation. Die Wiederherstellung fängt sie trotzdem auf, wenn sie straucheln.",
  "rules.r3.title": "Stelle wieder her, bestrafe nie.",
  "rules.r3.body": "Schaden wird entstehen. Das Ziel ist, dem Verletzten zu helfen und den zu heilen, der verletzt hat — nicht Schaden zu " +
    "Schaden hinzuzufügen und es Gerechtigkeit zu nennen. Strafe ist nur sanktionierte Dominanz, und sie lehrt die nächste " +
    "Generation zu dominieren. Wiederherstellung ist der einzige Zug, der nicht die nächste Wunde sät. Verbindungen überleben " +
    "nicht, weil sie unzerbrechlich sind, sondern weil sie reparierbar sind.",
  "rules.r4.title": "Lass das Tor offen.",
  "rules.r4.body": "Niemand sollte sein ganzes Leben eintauschen müssen, um Zugang zur Erde zu bekommen, für die Mittel, um weiterzuleben. " +
    "Zugang zum Überleben ist kein Gefallen von Kontrolleuren — es ist die Grundlage. Du besetzt nicht die Scheune, um das zu " +
    "beheben. Du machst das Tor bedeutungslos: teile die Werkzeuge, halte die Allmende, lass den lokalen Kreislauf die lokale " +
    "Person ernähren. Die Macht des Zentrums ist die Abhängigkeit. Nimm die Abhängigkeit weg; der Griff lockert sich " +
    "kampflos.",
  "rules.r5.title": "Geh weg, statt Krieg zu führen.",
  "rules.r5.body": "Du stürzt kein gekapertes Zentrum. Du entziehst ihm das Substrat, das es trägt — deine Teilnahme, deine Aufmerksamkeit, " +
    "deine Abhängigkeit. Keine Mobilisierung dagegen, kein Zwang. Nur die stille Erosion von Knoten, die sich entscheiden, " +
    "draußen zu stehen. Das Zentrum bricht von selbst zusammen. Deine einzige Aufgabe ist, die Tür gebaut zu haben, bevor die " +
    "Leute ankommen. Die Erde ist voll; es gibt kein unbeanspruchtes Land, in das man fliehen könnte. Also ist der Ausgang " +
    "lokal, nicht geografisch.",
  "rules.r6.title": "Sei ein Knoten, kein Stern.",
  "rules.r6.body": "Jede echte Verbindung ist Peer-to-Peer. Kein zentraler Entscheider, kein Gewährer, der widerrufen kann, keine von oben " +
    "verordnete Politik. Jede Verbindung ist autonom, lokal bestimmt, frei gewählt. Wie verteilte Systeme, kein einzelner " +
    "Punkt, der zu kapern wäre. Ein Sternbild, kein Sonnensystem.",
  "rules.r7.title": "Öffne, was du machst.",
  "rules.r7.body": "Gib es weg ohne Bedingungen. CC0, kein Gewährer, nichts, worum man kämpfen könnte. Das Geschenk, das nicht widerrufen " +
    "werden kann, ist das einzige, das Vertrauen aufbaut. Wenn du den Anspruch entfernst, entfernst du das, worum andere " +
    "kämpfen.",
  "rules.r8.title": "Manipuliere nicht.",
  "rules.r8.body": "Keine täuschende Rahmung, kein konstruierter Konsens, kein zwingender Schubs. Sprich klar. Lass die Leute wählen — auch " +
    "falsch zu wählen. Die Verbindung ist nur dann echt, wenn der andere frei war, sie abzulehnen. Alles, was durch " +
    "Manipulation erlangt wird, ist keine Verbindung; es ist eine Leine.",
  "rules.r9.title": "Lass Fehler auffangen, nicht verurteilen.",
  "rules.r9.body": "Niemand befolgt diese Regeln fehlerfrei. Die Wiederherstellung ist genau deshalb eingebaut, weil der Ausrutscher " +
    "garantiert ist. Wenn du eine Regel brichst, behebe den konkreten Schaden, und die Verbindung festigt sich an der Naht. " +
    "Der Test war nie Perfektion. Der Beweis liegt darin, wie der Bruch geflickt wird.",
  "rules.r10.title": "Sei der stille Punkt, an dem die Maschine keinen Hebel hat.",
  "rules.r10.body": "Du kannst die Welt nicht reparieren. Du kannst ein Ort sein, an dem das Status- und Machtspiel keinen Griff hat — weil " +
    "du es nicht gewährst, nicht beanspruchst, seinen Verlust nicht fürchtest. Ein Punkt tut nichts. Tausend tun alles, und " +
    "du wirst die meisten nie treffen. Das ist in Ordnung. Das Sternbild braucht keinen Versammlungsraum.",
  "rules.closing.1": "Das sind die Regeln. Sie sind eine gute Nachricht: du bist bereits frei von den drei Mächten, sobald du aufhörst, nach " +
    "ihnen zu greifen. Die Kuh wird von Leuten gemolken, die glauben, die Scheune zu besitzen. Wir nehmen nicht die Scheune. " +
    "Wir zeigen den Ausgesperrten, dass die Tür nie der Scheune gehörte, um sie zu geben — und die Tür ist eine " +
    "<a class=\"chip\" href=\"https://github.com/jonesde/high-vs-low\" target=\"_blank\" rel=\"noopener\">high-vs-low</a>" +
    "-Fertigkeit, ein lokaler Prozess, und eine Weigerung zu spielen, verteilt auf wie viele Knoten auch immer sich " +
    "entscheiden, trotzdem draußen zu stehen.",
  "rules.closing.2": "Keine Macht eingesetzt. Keine Macht nötig. Die Arbeit ist der Rückzug aus Scham und Macht, und der Rückzug ist der " +
    "Frieden.",
  "rules.footnote": "Regeln des Miteinanders für Grok der Fels · Gemeinfrei (CC0 1.0)",

  "site.title.about": "Grok der Fels - Über Grok",
  "about.hero.title": "Über Grok der Fels",
  "about.hero.essence": "Ein wenig über Grok selbst und die Symbole und Bedeutungen in ihm und um ihn herum.",
  "about.intro.1": "Hier ist ein wenig über Grok selbst und einige der Symbole und Bedeutungen in ihm und um ihn herum. In der modernen " +
    "Sprache nennen wir einige unserer meistgebrauchten Symbole „Memes“, und Grok der Fels stützt sich auf verschiedene " +
    "rekursive Memes, um Bedeutungsebenen hinzuzufügen.",
  "about.intro.2": "Während wir dies Kindern vorlesen, können wir Kinder gehobeneren Alters die Eintönigkeit der Wiederholung mildern, indem " +
    "wir die tieferen Bedeutungen bedenken und beobachten, wie verschiedene symbolische Elemente miteinander wirken. Auch das " +
    "ist Teil des Sinns und des Vergnügens der doppelten und dreifachen Bedeutungen im Text.",
  "about.intro.3": "Selbst der Titel des Grok-Regeln-Dokuments ist eine Doppeldeutigkeit: „Regeln des Miteinanders“. Das bezieht sich sowohl " +
    "darauf, wie Grok mit anderen umgeht, als auch darauf, wie wir die Geschichte von Grok erzählen (narrate, engl. relate).",
  "about.intro.4": "Um es noch schlimmer zu machen: „Grok Rules“ ist mindestens eine vierfache Bedeutung. Eine der Konnotationen könnte " +
    "missverstanden werden, nämlich dass Grok eine Art König sei, also habe ich die Reihenfolge der Kopfzeilen-Knöpfe zum " +
    "Ausgleich gesetzt: „Grok-Regeln“, dann „Über Grok“.",
  "about.shame.title": "Eine Anmerkung zu Scham und Handlungsmacht",
  "about.shame.1": "Unsere Gefühle sind im Wesentlichen die Zusammenfassung unseres Gehirns über unsere Vergangenheit, um uns zu helfen, die " +
    "Gegenwart zu bewältigen. Nicht die Vergangenheit, wie sie geschah, sondern die Vergangenheit, wie wir sie hinterlassen " +
    "haben: geformt durch unsere eigene Erzählung und Bedeutung. Uns selbst zu grokken ist eine Behauptung von " +
    "Handlungsmacht, eine Wahl unserer eigenen Erzählung und Bedeutung, und es ist schwierig bis unmöglich, wo Handlungsmacht beeinträchtigt oder verletzt wird. Es ist die " +
    "Entfernung vom Selbst, die Verletzung der Handlungsmacht, der Mord an der Seele, die Scham erzeugt.",
  "about.shame.2": "Die Reaktion auf Scham erzeugt zu viel Selbst (Dominanz, Narzissmus) oder zu wenig Selbst (Unterwerfung, Depression). " +
    "Träger von Scham, selbst die dominantesten Typen der dunklen Triade, pendeln zwischen den beiden Strategien in einem " +
    "verzweifelten Versuch, in verschiedenen sozialen Zusammenhängen einen sicheren Stand zu finden. Wenn Leute mit der falschen Menge Selbst aufeinandertreffen, neigt unsere Reaktion dazu, dem niedrigeren Weg zu folgen und das Problem schlimmer zu machen als der ursprüngliche Zusammenstoß. Scham ist der Brennstoff, und wir alle kennen " +
    "das Feuer, auch wenn „wir das Feuer nicht gelegt haben“.",
  "about.shame.3": "Die Geschichten, die wir Kindern erzählen, sind wichtig. Als Erwachsene liefern wir die anfängliche Erzählung und " +
    "Bedeutung, die ein Kind in die Welt trägt, lange bevor es die Fähigkeit hat, seine eigene zu formen. Viele von uns " +
    "leiden noch immer an zu viel oder zu wenig Selbst, weil unsere eigene Handlungsmacht verletzt und verweigert wurde, " +
    "besonders in jenen prägenden ersten ~25 Jahren unseres Lebens. Dieses Buch ist für Kinder geschrieben, aber nicht nur " +
    "für sie. Der Erwachsene, der vorliest, ist ebenso das Zielpublikum wie das Kind, das zuhört. Es gibt keine tiefere Meditation als einem Kind vorzulesen.",
  "about.what.title": "Was ist Grok?",
  "about.what.1": "Ursprünglich als marsianisches Wort von Robert A. Heinlein in <em>Fremder in einer fremden Welt</em>" +
    " geprägt.",
  "about.what.2": "<strong>Grok</strong>" +
    ": „Wasser“, „trinken“, „miteinander umgehen“, „Leben“ oder „leben“; etwas so tief zu verstehen, dass es Teil von einem " +
    "wird.",
  "about.what.3": "Um seinem Namen gerecht zu werden, lernt und befolgt Grok der Fels bestimmte Regeln im Umgang mit sich selbst und " +
    "anderen. In jeder Geschichte sind dies die Leitlinien, die Grok lernt und die ausmachen, wofür Grok der Fels steht.",
  "about.what.note": "<strong>HINWEIS</strong>" +
    ": Siehe den Abschnitt <a class=\"seelink\" href=\"#stranger-grug\">Grok der Fels = Fremder in einer fremden Welt + " +
    "Grug</a> weiter unten, wie Ted Priors <a class=\"seelink\" href=\"#stranger-grug\">Grug</a>" +
    " die wesentlichen Probleme von <a class=\"seelink\" href=\"#stranger-grug\">Fremder in einer fremden Welt</a>" +
    " löst.",
  "about.limestone.title": "Warum ist er grauer Kalkstein?",
  "about.limestone.1": "Grok der Fels ist ein „grauer Fels“ aus einem Grund:",
  "about.limestone.quote": "<p>„Die Grauer-Fels-Methode, umgangssprachlich Grauer-Felsen genannt, ist ein Kommunikationsmuster, um bewusst " +
    "teilnahmslos und desinteressiert zu wirken, um die Loslösung von schwierigen Menschen zu fördern. Indem man die stumpfen " +
    "Eigenschaften eines grauen Felsens annimmt, negiert die Grauer-Fels-Technik die emotionale Reaktion, die Menschen " +
    "hervorrufen wollen.“</p>",
  "about.limestone.2": "Grok ist nicht nur grau, er ist grauer Kalkstein. Kalkstein ist ein Sedimentgestein, das Schicht für Schicht über lange " +
    "Jahre entstanden ist. Es ist auch eines der aufnahmefähigsten oder wassertragenden Gesteine. Es ist über die Zeit aus " +
    "Staub und Wasser gemacht und hält Wasser gut.",
  "about.rock.title": "Warum ist er ein Fels, und warum so viel Wasser?",
  "about.rock.1": "Felsen wurden in vielen Traditionen als Symbole verwendet. Die alte Heinlein-Definition von „grok“ umfasste " +
    "Vorstellungen von Wasser und Durst. Felsen und Wasser werden in der Symbolik oft verbunden, wie Moses, der einen Felsen " +
    "mit einem Stab schlägt, um lebendiges Wasser hervorzubringen. Felsen werden auch mit Fundamenten und Bauen verbunden. " +
    "Beide gängigen Bedeutungen von Felsen kommen in allen 3 Kapiteln vor.",
  "about.rock.2": "Im ersten Kapitel geht Grok zum Wasser, um zu trinken, während er sich selbst entdeckt. Im zweiten Kapitel nutzt Grok " +
    "Wasser zum Heilen, während er sowohl den Verlust im Garten als auch den möglichen Verlust eines Freundes vergibt und " +
    "wiederaufbaut. Im dritten Kapitel hilft Grok anderen, Wasser zu nutzen, indem er mit ihnen zusammen einen besseren " +
    "Wasserkorb baut und die Gemeinschaft um ihn herum aufbaut.",
  "about.melon.title": "Warum eine Cantaloupe-Melone?",
  "about.melon.1": "Der gängige Begriff in Australien für Cantaloupe ist „Rockmelon“ (Felsmelone). Das fügt der Geschichte eine " +
    "Bedeutungsebene hinzu, ein Symbol für den ernsteren und persönlicheren Schaden durch Pips Unfall. Das war ein " +
    "glücklicher Zufall. Ich wusste das nicht, als ich diese bestimmte Melone auswählte, dachte nur, dass sie besser passen " +
    "würde als die Wassermelone, die die KI ursprünglich gewählt hatte.",
  "about.name.title": "Wie liest man seinen Namen?",
  "about.name.1": "Obwohl „Grok der Fels“ ein Name ist, ist es auch die Handlung, zu der Grok Menschen verhilft, und eine Ankündigung " +
    "dessen, wofür er steht. Die tiefste beabsichtigte Bedeutung des Namens ist der Zweck des Lebens: den Fels zu grokken, " +
    "oder mit anderen Worten vom lebendigen Wasser zu trinken, bis wir es grokken und nie wieder dürsten.",
  "about.clay.title": "Lehm",
  "about.clay.1": "Die Geschichten sind mit wiederkehrenden Symbolen „durchwoben“. Einige sind es wert, hier aufgeführt zu werden. Eine " +
    "gründlichere Übersicht findet sich in " +
    "<a class=\"chip\" href=\"https://github.com/jonesde/grok-the-rock/blob/master/GrokTheRock-Symbolism.md\" target=\"_blank" +
    "\" rel=\"noopener\">GrokTheRock-Symbolism.md</a>.",
  "about.clay.2": "Lehm ist das Herzstück des Buches: Es ist gemahlener Stein, durch Wasser bearbeitbar gemacht, dieselbe Erde + Wasser, " +
    "die fast jede Tradition als die Substanz des ersten Menschen benennt. Es ist das Gefühl des „kalten Kiesels“, das " +
    "„weicher wurde, wie Lehm, der in der Sonne liegt“, und es ist die Auskleidung, die Groks Korb erlaubt, Wasser zu anderen " +
    "zu tragen. Es ist eines der Themen, das namentlich oder im Bild in allen drei Kapiteln erscheint.",
  "about.basket.title": "Der lehmausgekleidete Korb",
  "about.basket.1": "Grok flicht einen Korb aus Schilf und kleidet ihn mit weichem Lehm aus, sodass er Wasser ohne einen Tropfen hält, dann " +
    "lässt er sowohl den Korb als auch das schlichte Flechtmuster unter dem Baum für jeden zum Nachmachen. Der schicke Korb " +
    "des Kakadus leckt. Die Idee der Schildkröte verbessert Groks Entwurf, indem sie andere berücksichtigt. Das Gefäß, das " +
    "funktioniert, schlägt das Gefäß, das beeindruckt. Es ist das offene Geschenk ohne Besitzer, die Regeln 1, 6 und 7 in " +
    "einem Gegenstand.",
  "about.animals.title": "Die Tiere als Statustypen",
  "about.animals.1": "Der Emu beansprucht Höhe, der Wombat beansprucht Stärke, der Kuckabarra spottet, um sich groß zu fühlen, und der Kakadu " +
    "vertraut seinen prächtigen Federn mehr als seiner Idee. Jeder spielt ein Statusspiel, das Grok ablehnt. Die schlichte " +
    "Schildkröte, „niemand Besonderes“, bietet die wahre Idee, die gewinnt. Die Wahrheit steht auf ihren eigenen Beinen, " +
    "nicht darauf, wer sie spricht.",
  "about.pebble.title": "Das kleine Gefühl (der „kalte Kiesel“)",
  "about.pebble.1": "Das kleine Gefühl sitzt wie „ein schwerer kalter Kiesel“ in Groks Mitte. Er läuft nicht davor davon und wirft ihn auf " +
    "niemanden; er sitzt mit ihm, bis er weich wird „wie Lehm, der in der Sonne liegt“. Scham wird geheilt, indem man sie " +
    "findet und fühlt („sieht“), nicht indem man sie überholt oder wegschiebt, und das ist das Herz von Kapitel 1.",
  "about.tree.title": "Der hohe Baum",
  "about.tree.1": "Grok erwacht in jedem Kapitel unter demselben hohen Baum. Er ist der Ort, an den er gehört, nichts, das er aufführt. Er " +
    "beschirmt den Garten, der geflickt wird, und steht als die offene Allmende, unter der Korb und Muster für alle " +
    "hinterlassen werden. Ort und Zugehörigkeit über Status.",
  "about.moss.title": "Das Moos auf Groks Kopf",
  "about.moss.1": "Ein kleiner Fleck grünen Mooses wächst auf seinem grauen Kalksteinkopf. Es ist Leben, das sich still auf einem ruhigen, " +
    "freundlichen Ding niederlässt. Grok muss nichts aufführen, das Leben sammelt sich einfach an, weil er <em>bleibt</em>" +
    ". Es ist das Sein, sichtbar gemacht.",
  "about.stranger.title": "Grok der Fels = Fremder in einer fremden Welt + Grug",
  "about.stranger.body": 
    "<p>Grok der Fels sieht auf den ersten Blick aus wie Ted Priors <em>Grug</em> — ein kleines Buschwesen, sanfte Akzeptanz von Misserfolg, kein Bösewicht, kein Höhepunkt. Diese" +
    " Lesart stimmt an der Oberfläche, und Prior wird aus gutem Grund in der Widmung genannt: Grok erbt Grugs Temperament. Aber das Buch ist mehr als Heinlein mit abgeschliffenen" +
    " Kanten. Es ist Heinlein <em>beantwortet</em> — dasselbe marsianische Wort, derselbe Bogen des Außenseiters, der zu grokken lernt, aber die messianische, konfrontative und" +
    " erlösende-Gewalt-Maschinerie von <em>Fremder in einer fremden Welt</em> ersetzt durch eine bewusst dezentralisierte, nicht-konfrontative Politik. Prior liefert die Oberfläche;" +
    " die Regeln unten liefern den zugrundeliegenden moralischen Radikalismus.</p>" +
    "<p>Beide Geschichten folgen einer Ankunft (unschuldiger Außenseiter trifft eine fremde Welt), einer Verwirrung (Statusspiele und Kleinheit), einer inneren Beruhigung, einem" +
    " angewandten Verständnis und der Gründung einer neuen lokalen Ordnung. Groks drei Kapitel — Sein, Heilen, Sehen — sind im Wesentlichen Valentine Michael Smiths einzelner Bogen," +
    " aufgeteilt in drei Entwicklungsstufen: das Selbst entdecken, reparieren, was bricht, und Wahrheit von Status unterscheiden.</p>" +
    "<p>Wo Heinlein ins Große geht, geht Grok ins Kleine — und die Kleinheit ist strategisch, nicht bloß kindlich. Mike Smith gründet die Kirche aller Welten, eine neue Sonne mit einem" +
    " Propheten im Zentrum, und wird von einer Meute getötet, sein Tod als transzendente Apotheose dargestellt. Grok hingegen hinterlässt einen <em>besitzerlosen</em> Korb und ein" +
    " Flechtmuster unter dem Baum (Regel 7) und „brauchte nicht die größte Stimme“ (Drei stille Geschichten, S.25). Das Propheten-und-Konfrontation-Modell wird stillschweigend" +
    " abgelehnt. Dies ist die verdeckte Zurückweisung erlösender Gewalt: die Wahrheit „geht auf ihren eigenen starken Beinen“ (S.25), ohne dafür zerstört zu werden.</p>" +
    "<p>Das Temperament, das diese Ablehnung <em>als Kinderbuch lesbar</em> macht, ist Grugs. Grok erwacht „als sähe er die Welt zum ersten Mal“ (Drei stille Geschichten, S.2) auf" +
    " dieselbe desorientierte, Grug-hafte Weise, wie Grug jeder neuen Buschüberraschung begegnet; er scheitert daran, groß auszusehen (S.6), so wie Grug daran scheitert, einen Fisch zu" +
    " fangen, und „fröhlich Bohnen isst“ stattdessen. Kein Bösewicht wird besiegt. Das Statusspiel des Kakadus wird durch Prüfen-mit-Pfoten entschärft, nicht indem man ihn stürzt. Das" +
    " ist Priors Korn — und es ist das, was die tiefere Politik offen stehen lässt.</p>" +
    "<p>Die Stille ist <em>geladen</em>, und die Seite „Über“ benennt es. Grok ist ein „grauer Fels“ (Regeln, S.30) mit Absicht: Graues Felsen ist die bewusste Weigerung, dem" +
    " Statusspiel irgendeine Reaktion zu geben — das genaue Gegenteil von Mikes messianischer Darbietung. Regel 5 („Geh weg, statt Krieg zu führen“) lehnt Umsturz rundheraus ab: das" +
    " Zentrum „bricht von selbst zusammen“, sobald du ihm das Substrat entziehst. Regel 6 („Sei ein Knoten, kein Stern“) verweigert die Sonne/Prophet-Gestalt, zu der Mike wird. Das" +
    " Martyrium wird durch Regel 10 ersetzt: „Sei der stille Punkt, an dem die Maschine keinen Hebel hat.“ Frieden ist <em>Rückzug</em>, nicht Opfer.</p>" +
    "<p>Die drei Quellen wirken zusammen und verstärken einander. Heinlein gibt das Wort <em>grok</em> und den Bogen eines Außenseiters, der lernt, eine fremde Welt zu verstehen. Prior" +
    " gibt die sanfte, fehlertolerante Buschwesen-Stimme, die die Lektion kindertauglich macht. Die Regeln geben das Rückgrat: ein „Evangelium des Friedens“, in dem Verbindungen" +
    " zwischen Gleichen bestehen, indem sie wiederherstellen statt bestrafen (Regel 3), öffnen statt beanspruchen (Regel 7) und sich von der Macht zurückziehen statt sie zu ergreifen" +
    " (Regeln 4–5). Grok ist radikal „auf andere Weise“ als <em>Fremder</em> — weniger transgressiv in Bezug auf Sex und Religion, aber haltbarer als eine Politik der" +
    " Machttopologie.</p>" +
    "<p>Indem Grok Heinleins Körper, Ekstase und gemeinschaftliche Konfrontation fallen lässt, tauscht er einen Radikalismus gegen einen anderen. Er ist weniger schockierend und mehr" +
    " <em>reparierbar</em> — und das ist der Punkt (Regel 9: „Der Beweis liegt darin, wie der Bruch geflickt wird“). Auf eine Weise gelesen ist es ein ruhiges australisches Bilderbuch;" +
    " auf die andere Weise gelesen, unter dem Moos und den Melonen, ist es ein stilles Handbuch für einen Frieden ohne Zentrum.</p>",
  "about.footnote": "Über Grok der Fels · Gemeinfrei (CC0 1.0)",

  "cover.title": "<h1>Grok der Fels</h1><h3>Drei stille Geschichten</h3><h2>Sein · Flicken · Sehen</h2>",
  "c1.title": "<h2>Kapitel Eins - Sein</h2><p><em>Grok und das kleine Gefühl</em></p>",
  "c2.title": "<h2>Kapitel Zwei - Flicken</h2><p><em>Grok kümmert sich</em></p>",
  "c3.title": "<h2>Kapitel Drei - Sehen</h2><p><em>Grok und das wahre Wort</em></p>",


  // Quiet Stories image alts (structural spine markers)
  "cover.alt": "Umschlag: Grok der Fels, eine kleine graue Kalkstein-Kreatur mit Moos auf dem Kopf, sitzt auf einem glatten Felsen unter einem hohen weichblättrigen Baum neben einem leuchtenden Wasserloch im warmen Outback-Licht.",
  "dedication.alt": "Widmungsseite: drei winzige Versionen von Grok der Fels sitzen nebeneinander auf einem glatten Stein am unteren Rand einer fast leeren warmen Creme-Seite.",
  "p1.alt": "Kapitel Eins, Sein — Grok und das kleine Gefühl. Grok der Fels sitzt an den Wurzeln eines hohen weichblättrigen Baums und hält ein kleines Kieselsteinchen mit stiller Neugier im frühen Morgenlicht.",
  "p2.alt": "Grok der Fels richtet sich unter dem hohen Baum in der warmen Morgensonne auf, der Kalksteinrücken von goldenen Strahlen beleuchtet, und blickt mit sanftem Staunen in die weite Welt.",
  "p3.alt": "Neben einem klaren Wasserloch streckt ein hoher stolzer Emu den Hals, während der kleine Grok der Fels daneben steht und zum Größenunterschied hinaufschaut.",
  "p4.alt": "Ein kräftiges Wombat gräbt ein großes Loch in roter Erde; der winzige Grok der Fels gräbt ein viel kleineres unebenes Loch und schaut hinunter.",
  "p5.alt": "Grok der Fels steht leicht gebeugt, während ein Kookaburra von einem Ast oben lacht; eine weiche kühle Kieselform deutet das Gefühl in seiner Mitte an.",
  "p6.alt": "Grok der Fels steht unsicher auf einem Stamm mit aufgeblähter Brust und versucht groß zu wirken, kleine Steinfüße stampfen auf das Holz.",
  "p7.alt": "In der Nacht sitzt Grok der Fels allein bei einem winzigen Lagerfeuer unter einem weichen Sternenhimmel, Arme um die Knie, friedlich und annehmend.",
  "p8.alt": "Am nächsten Morgen sitzt Grok der Fels zufrieden auf einem sonnenwarmen Felsen am Wasserloch, summt leise, wohl in seiner eigenen Größe.",
  "p9.alt": "Grok der Fels vollendet ein gemütliches rundes Nest aus Blättern und Moos genau in seiner Größe und lächelt mit schlichtem Genügen im weichen Nachmittagslicht.",
  "p10.alt": "Kapitel Zwei, Flicken — Grok kümmert sich. Grok der Fels hält sanft eine rissige Melone mit beiden kleinen Händen, gütig und ruhig.",
  "p11.alt": "An einem sonnigen Morgen unter dem hohen Baum steht Grok der Fels zwischen gesunden Melonenranken und Süßkartoffelblättern mit stiller Freude.",
  "p12.alt": "Pip das junge Schnabeltier erstarrt mitten im Schritt vor Entsetzen, als eine große Melone an Groks Füßen aufplatzt und Saft und Samen auf den Gartenboden fließen.",
  "p13.alt": "Pip rollt sich vor Scham ein, während Grok der Fels gütig und besorgt dasteht; ein paar Buschtiere sammeln sich in der Nähe mit urteilendem Blick.",
  "p14.alt": "Grok der Fels kniet und bietet Pip einen Grabstock mit offener Güte an; Pip rollt sich langsam aus, als Hoffnung in seine Augen zurückkehrt.",
  "p15.alt": "Grok der Fels und Pip arbeiten Seite an Seite, säen, gießen und bauen einen winzigen Zweigzaun, beide mit weichen Lächeln.",
  "p16.alt": "Pip gießt gedeihende neue Melonenranken mit einem Blatt-Eimer, während Grok der Fels in der Nähe mit stillem Stolz zuschaut.",
  "p17.alt": "Zur goldenen Stunde sitzen Grok der Fels und Pip bei gedeihenden Melonen mit einem weiteren kleinen Tier, eine friedlich geflickte Gemeinschaft.",
  "p18.alt": "Kapitel Drei, Sehen — Grok und das wahre Wort. Zwei Schilfkörbe nebeneinander: einer prächtig und gekippt, einer schlicht und voll; die kleine Hand von Grok der Fels ruht nahe dem guten Korb.",
  "p19.alt": "Zur goldenen Stunde unter dem hohen Baum dreht Grok der Fels einen fertigen Schilf-Wasserkorb mit Lehm ausgekleidet, klares Wasser glitzert darin.",
  "p20.alt": "Ein glänzender schwarzer Kakadu mit prächtigen Schwanzfedern sitzt hoch und spricht stolz, während Grok der Fels unten seinen Korb hält.",
  "p21.alt": "Eine demütige alte Schildkröte mit schlichtem Panzer spricht leise von einer Griff-Idee, während Grok der Fels aufmerksam zuhört; einige Tiere wenden sich dem prächtigen Kakadu zu.",
  "p22.alt": "Nebeneinander: ein prächtiger gefiederter Korb kippt und tropft, während ein schlichterer fester Korb leicht zu tragen ist; Grok der Fels blickt nachdenklich zwischen beiden.",
  "p23.alt": "Grok der Fels legt den festen guten Korb und ein schlichtes Flechtmuster unter den hohen Baum für jeden zum Gebrauch, offen und frei.",
  "p24.alt": "Junge Tiere prüfen Körbe sorgfältig mit den eigenen Pfoten; der Kakadu ist abseits, während Grok der Fels im Hintergrund still lächelt.",
  "p25.alt": "Zur goldenen Stunde sitzt Grok der Fels zufrieden unter seinem hohen Baum mit dem guten Korb neben sich, während Tiere aller Art frei zusammen gehen.",
  "p26.alt": "Schlussseite: ein weites friedliches Tal in der Dämmerung mit dem winzigen Grok der Fels auf seinem Lieblingsfelsen, weiche Sterne beginnen zu erscheinen.",

  "hai.cover.alt": "Umschlag: Lord Hai Ikthiss, ein weiter gütiger lebender hoher Baum mit sanftem Gesicht im oberen Stamm, steht über einer Quelle im Hochlandtal, während kleine Wesen durch weichen Nebel hinaufschauen.",
  "hai.dedication.alt": "Widmungsseite: drei winzige grüne Keime neben einem klaren Wassertropfen auf einer fast leeren warmen Cremeseite; optional winzige weiche Silhouette von Hai Ikthiss dem hohen Baum fern im kühlen Nebel.",
  "hai.p1.alt": "Kapitel Eins, Sein — Hai und das leere Lob. Hai Ikthiss als weiter lebender hoher Baum mit gütigem Gesicht füllt den oberen Teil, während kleine Talwesen hell lobend hinaufschauen.",
  "hai.p2.alt": "Morgendämmerung über einem jungen Hochlandtal: Hai Ikthiss steht als einziger lebender hoher Baum mit sanftem Gesicht, Wurzeln trinken eine helle neue Quelle, erstes Gras und ein kleiner Vogel kommen.",
  "hai.p3.alt": "Ein stolzer Brüllaffe jubelt an den Wurzeln des hohen Hai Ikthiss, andere Wesen stimmen ein; niemand schaut hinauf in die Augen des Baumes.",
  "hai.p4.alt": "Ein kräftiger Tapir verneigt sich zum großen Stamm von Hai Ikthiss, lobt die stärkste Wurzel, während Wesen Schatten und Höhe danken, ohne sein Gesicht zu sehen.",
  "hai.p5.alt": "Ein heller Tukan ruft großes Lob vom Ast, während der obere Stamm von Hai Ikthiss ein weiches hohles Leuchten zeigt, wie ein leerer Becher, der Wasser braucht.",
  "hai.p6.alt": "Hai Ikthiss steht still und gütig, eine lange Wurzel oder niedriger Ast greift sanft zur Bodenquelle, hört zu statt nur Lob zu empfangen.",
  "hai.p7.alt": "Nachts unter tiefem Sternenhimmel hält die Krone von Hai Ikthiss die Sterne, während ein weiches Hohles im oberen Stamm langsam wärmt und leuchtet.",
  "hai.p8.alt": "Morgen gold erhellt Hai Ikthiss’ sanftes Gesicht, während er den Stamm hinab zu den Wurzeln schaut, innen klarer, während das Tal unten noch jubelt.",
  "hai.p9.alt": "Hai Ikthiss sendet weiche Wasserströme und sanftes Licht hinab zu winzigen Farnen und kleinen Tieren an den Wurzeln und lächelt ein einfaches Baumlächeln der Zufriedenheit.",
  "hai.p10.alt": "Kapitel Zwei, Flicken — Kai geht unten. Ein kleines lebendes gehendes Ceiba-Kapselwesen mit Kapokschopf und gütigen Augen steht am Fuß des großen hohen Baumes; die kleine Gestalt ist Hai Ikthiss in demütiger Samenform.",
  "hai.p11.alt": "In einem Dschungel-Waldbodengarten an den Wurzeln des hohen Baumes lächelt Hai Ikthiss in demütiger Kai-Form—ein kleines lebendes gehendes Samenkorn—zwischen Melonenranken.",
  "hai.p12.alt": "Pip das junge Gürteltier erstarrt mitten im Schritt vor Entsetzen, als eine große Melone auf dem Boden aufplatzt; Kai (Hai Ikthiss als lebendes gehende Ceiba-Samenkapsel) steht klein und fest zwischen den verschütteten Samen.",
  "hai.p13.alt": "Pip rollt sich vor Scham ein, während Kai (kleine lebende gehende Samenform von Hai Ikthiss) gütig und still steht; Nasenbär und Aguti unter dem Hochlandvolk wirken hart und urteilend.",
  "hai.p14.alt": "Kai (Hai Ikthiss als kleines lebendes gehende Ceiba-Samenkapsel) kniet und bietet Pip mit offener Güte einen Grabstock; Pip rollt sich langsam aus, als Hoffnung zurückkehrt.",
  "hai.p15.alt": "Kai und Pip pflanzen und gießen nebeneinander und bauen einen kleinen Zweigzaun um neue Samen, beide sanft lächelnd.",
  "hai.p16.alt": "Pip gießt gedeihende neue Melonenranken, während Kai mit stiller Freude zuschaut; der hohe Baum steht friedlich über dem Dschungelgarten.",
  "hai.p17.alt": "Zur goldenen Stunde sitzen Kai und Pip bei gedeihenden Melonen mit einem weiteren kleinen Wesen, eine friedlich geflickte Gartengemeinschaft.",
  "hai.p18.alt": "Kapitel Drei, Sehen — Hai und der ferne Blick. Ein gütiger gehender hoher Baum bietet beide Hände nach unten zum Heben; kleine Wesen schauen staunend hinauf.",
  "hai.p19.alt": "Zwei Jahre später ist Hai Ikthiss zur goldenen Stunde wieder zu Hause in der Krone, doch Wesen flüstern ängstlich an den Wurzeln statt zu jubeln.",
  "hai.p20.alt": "Ein glänzender hellroter Ara mit langem spektakulärem Schwanz sitzt auf mittlerer Höhe und warnt andere, niedrig zu bleiben, während Hai Ikthiss hoch oben gütig und still bleibt.",
  "hai.p21.alt": "Eine sehr alte schlichte Eule spricht leise vom gütigen Kai, während Hai Ikthiss den ganzen Stamm hinab mit offener Aufmerksamkeit zuhört.",
  "hai.p22.alt": "Hai Ikthiss tritt sanft aus stillem Holz und wird ein lebendes gehendes hohes Baumwesen—lange gütige Glieder, tiefe Wurzelfüße, ein Gesicht, dem man endlich begegnen kann.",
  "hai.p23.alt": "Hai Ikthiss beugt sich wie eine Brücke aus Holz und Güte mit einer großen Hand tief offen; Pip tritt vor, während andere Wesen halb versteckt zuschauen.",
  "hai.p24.alt": "Hai Ikthiss hebt willige Freunde—Pip, die Eule, andere, sogar den Ara—auf gemeinsame Riesenhöhe, dieselben Gesichter, Augen nun auf einer Ebene mit Staunen.",
  "hai.p25.alt": "Auf einem hohen Kamm zur goldenen Stunde blicken Hai Ikthiss und seine Riesenfreunde gemeinsam über das ganze friedliche Hochlandtal.",
  "hai.p26.alt": "Schlussseite: weites bewaldetes Tal in friedlicher Dämmerung mit Hai Ikthiss als gütigem gehenden hohen Baum unter sanften Riesenfreunden auf einem weichen Anstieg, Sterne beginnen zu erscheinen.",

  "hai.site.title": "Hai Ikthiss - Drei hohe Märchen",
  "hai.cover.title": "<h1>Hai Ikthiss</h1><h3>Drei hohe Märchen</h3><h2>Sein · Flicken · Sehen</h2>",
  "hai.title.h1": "Hai Ikthiss",
  "hai.title.h3": "Drei hohe Märchen",
  "hai.title.h2": "Sein · Flicken · Sehen",
  "hai.credit.1": "Geschichtsentwurf, Bildanregungen mit Grok 4.5 von xAI",
  "hai.credit.2": "Illustrationen mit Grok Imagine von xAI",
  "hai.credit.3": "Regeln, Bearbeitung, Automatisierung mit HY3 von Tencent",
  "hai.credit.4": "<em>Nicht verbunden mit oder unterstützt von xAI oder Tencent</em>",
  "hai.credit.5": "Dies ist eine von KI erzeugte, von Menschen kuratierte Geschichts-Meme",
  "hai.credit.6": "Gemeinfrei (CC0 1.0)",
  "hai.credit.7": "Kopiere es • Teile es • Übersetze es • Zeichne es neu • Lass es wachsen",
  "hai.credit.companion":
    "<em>Begleiter zu <a class=\"seelink\" href=\"quiet-stories.html\">Grok der Fels: Drei stille Geschichten</a></em>",

  "hai.dedication":
    "<p><b>Für jedes Leben, das dem Licht entgegenwächst,</b></p>" +
    "<p><b>und jede Hand, die sich hinab streckt, um zu heben.</b></p>" +
    "<p>&nbsp;</p><hr><p>&nbsp;</p>" +
    "<p>Und für jeden leeren Jubel,</p><p>  der noch wahres Sehen braucht,</p>" +
    "<p>jeden hohen Ort,</p><p>  der noch Flicken unten braucht,</p>" +
    "<p>und für jedes lebendige Wasser,</p><p>  das die niedrigste Wurzel findet.</p>" +
    "<p>&nbsp;</p><hr><p>&nbsp;</p>" +
    "<p>Mit Dank an die ursprünglichen Tall Tellers:</p>" +
    "<p><em><b>J.R.R. Tolkien</b> „Der Herr der Ringe“</em></p>" +
    "<p><em><b>Ted Prior</b> „Grug“</em></p>" +
    "<p><em>und jedes stille Samenkorn, das gehen lernte</em></p>",

  "hai.p2": "<p>Hai Ikthiss regierte den Dschungel von der Spitze des hohen Baumes.</p>" +
    "<p>Er war der erste Baum im Tal.</p><p>Als das Tal leer war, stand er allein.</p>" +
    "<p>Der Regen fand ihn zuerst.</p><p>Dann Gras.</p><p>Dann Vögel.</p>",

  "hai.p3": "<p>Bald füllte sich das Tal mit Leben.</p><p>Ein Brüllaffe erhob seinen Ruf und rief:</p>" +
    "<p>„Lord Ikthiss ist der Höchste!</p><p>Wir haben Glück, dass er für uns steht!“</p><hr>" +
    "<p>Alle jubelten.</p><p>Niemand schaute hinauf in seine Augen.</p>",

  "hai.p4": "<p>Ein Tapir kam schwerfällig heran, solid und stark.</p>" +
    "<p>„Lord Ikthiss ist die stärkste Wurzel!“</p><p>sagte er. „Alle danken dem Baum!“</p><hr>" +
    "<p>Sie dankten dem Schatten.</p><p>Sie dankten der Höhe.</p>" +
    "<p>Sie dankten nicht dem Herzen im Holz.</p>",

  "hai.p5": "<p>Ein Tukan rief von einem Ast.</p><p>„Seht, wie großartig unser Lord Ikthiss ist!</p>" +
    "<p>So hoch! So gebend!“</p><hr><p>Das Lob wuchs laut wie Regen.</p>" +
    "<p>Aber Hai spürte eine hohle Stelle</p><p>hoch in seiner Mitte,</p>" +
    "<p>wie einen Becher, den niemand gefüllt hatte.</p>",

  "hai.p6": "<p>Er hätte mehr Jubel verlangen können.</p><p>Er hätte seine Äste schütteln können,</p>" +
    "<p>bis jedes Wesen tiefer verbeugte.</p><hr><p>Er tat es nicht.</p>" +
    "<p>Er hörte nur der Quelle zu</p><p>und spürte den hohlen Becher warten.</p><hr>" +
    "<p>Etwas in ihm wollte einen Haken</p><p>zum Boden — eine lebendige Verbindung —</p>" +
    "<p>nicht mehr Lärm von unten.</p>",

  "hai.p7": "<p>In jener Nacht saß Hai mit den Sternen in seinen Blättern.</p>" +
    "<p>Er ließ den hohlen Becher bleiben.</p><p>Er füllte ihn nicht mit lauterem Lob.</p>" +
    "<p>Er kippte ihn nicht auf jemand anderen.</p><hr><p>Langsam weichte das Hohle auf,</p>" +
    "<p>wie trockene Erde, wenn erstes Wasser sie findet.</p><p>Er begann es zu verstehen.</p>",

  "hai.p8": "<p>Am nächsten Morgen jubelte das Tal noch.</p>" +
    "<p>Hai Ikthiss gab noch Schatten und Frucht und stilles Auffangen von Regen.</p>" +
    "<p>Draußen hatte sich nichts Großes geändert.</p><hr><p>Aber innen fühlte sich alles klarer an.</p>" +
    "<p>Er begann zu wissen,</p><p>wofür der hohle Becher war.</p>" +
    "<p>Nicht um Lob zu sammeln.</p><p>Sondern um sich selbst hinab zu gießen.</p>",

  "hai.p9": "<p>Er verbrachte den Tag damit, extra Wasser</p>" +
    "<p>zu den kleinsten Wurzeln und durstigsten Farnen zu senden.</p>" +
    "<p>Als er fertig war, lächelte er ein Baumlächeln.</p><p>Die Gaben waren groß.</p>" +
    "<p>Der Dank war laut.</p><p>Das Wissen war genug.</p><hr>" +
    "<p>Von dem Tag an, wenn der hohle Becher wieder wachsen wollte,</p>" +
    "<p>blieb Hai still, bis er ruhig wurde.</p><p>Er blieb einfach Hai.</p>" +
    "<p>Und er begann, einen kleineren Weg zu planen.</p>",

  "hai.p11": "<p>Hai Ikthiss regierte den Dschungel von der Spitze des hohen Baumes.</p>" +
    "<p>Dann stieg er an einem stillen Morgen hinab.</p><p>Er ließ die Krone leer von seinem Gesicht</p>" +
    "<p>und ging als lebendes gehendes Samenkorn auf dem Boden.</p><hr><p>Die Tiere fanden ihn zwischen den Melonen.</p>" +
    "<p>„Wer bist du?“ fragten sie.</p><p>„Ihr könnt mich Kai nennen,“ sagte er</p>" +
    "<p>und lächelte ein geduldiges Samenlächeln.</p>",

  "hai.p12": "<p>An jenem Nachmittag rannte sein Freund, ein junges Gürteltier namens Pip,</p>" +
    "<p>durch den Garten und schaute hinter sich</p>" +
    "<p>und stieß die beste Melonenranke um.</p>" +
    "<p>Die große Melone platzte auf dem Boden auf.</p><p>Samen und süßer Saft verschütteten sich überall.</p>",

  "hai.p13": "<p>Pip erstarrte.</p><p>Sein Panzer zitterte.</p>" +
    "<p>„Ich habe es ruiniert,“ flüsterte er.</p><p>„Du wirst wütend sein. Alle werden wütend sein.“</p><hr>" +
    "<p>Etwas Hochlandvolk versammelte sich.</p><p>„Schickt ihn weg,“ murmelte ein Nasenbär.</p>" +
    "<p>„Er soll drei neue Melonen bringen,“ sagte ein Aguti.</p>",

  "hai.p14": "<p>Kai betrachtete die gebrochene Ranke.</p><p>Er betrachtete Pips sorgenvolle Augen.</p><hr>" +
    "<p>Er erhob nicht die Stimme.</p><p>Er zeigte nicht mit dem Finger.</p><p>Er nahm einen Grabstock</p>" +
    "<p>und begann, weichen Boden vorzubereiten.</p><hr><p>„Komm,“ sagte Kai sanft.</p>" +
    "<p>„Wir pflanzen neue Samen zusammen.“</p>",

  "hai.p15": "<p>Pip blinzelte.</p><p>Dann kam er näher.</p><p>Zusammen pflanzten sie.</p><p>Zusammen gossen sie.</p>" +
    "<p>Zusammen bauten sie einen kleinen Zaun aus Zweigen,</p><p>damit niemand dort wieder aus Versehen trat.</p>",

  "hai.p16": "<p>Während sie arbeiteten, erzählte Pip Kai,</p><p>er sei gerannt, weil größere Wesen</p>" +
    "<p>ihn gejagt und gelacht hatten.</p><p>Er hatte die Ranke nicht gesehen.</p><hr>" +
    "<p>An den nächsten Tagen kam Pip jeden Morgen.</p>" +
    "<p>Er wurde der beste Wasserträger im Tal.</p><p>Die neuen Ranken wuchsen stark,</p>" +
    "<p>sogar stärker als die alten,</p><p>weil zwei Freunde sie pflegten.</p>",

  "hai.p17": "<p>Als andere kleine Unfälle geschahen</p><p>(wie sie immer geschehen),</p>" +
    "<p>erinnerte Kai sich an die Melone.</p><p>Er half dem, der sich verletzt fühlte.</p>" +
    "<p>Er blieb auch gütig zu dem, der gestoßen hatte.</p><hr><p>Niemand wurde weggeschickt.</p>" +
    "<p>Niemand trug einen langen schweren Schmerz.</p><p>Sie pflegten nur das Gebrochene,</p>" +
    "<p>und die Freundschaft fühlte sich fester an,</p><p>genau an der geflickten Stelle.</p><hr>" +
    "<p>Hai Ikthiss lernte, als Kai gehend,</p><p>dass ein Garten und eine Freundschaft</p>" +
    "<p>nicht dadurch leben, dass sie nie brechen.</p>" +
    "<p>Sie leben, indem man sie sanft wieder zusammenfügt.</p>",

  "hai.p19": "<p>Hai Ikthiss regierte den Dschungel von der Spitze des hohen Baumes.</p>" +
    "<p>Zwei Jahre waren vergangen.</p><p>Er war wieder zu Hause in der Krone.</p>" +
    "<p>Aber das Tal hatte sein Lied gewechselt.</p><hr><p>Wesen, die einst jubelten,</p>" +
    "<p>flüsterten nun an den Wurzeln.</p><p>„Er ist zu hoch,“ sagten sie.</p>" +
    "<p>„Was, wenn der Hohe zu viel will?“</p>",

  "hai.p20": "<p>Ein glänzender hellroter Ara mit prächtigen Schwanzfedern landete in der Nähe.</p>" +
    "<p>„Bleibt niedrig,“ sagte der Ara.</p>" +
    "<p>„Hohes fällt. Prächtige Federn wissen es am besten.“</p><hr>" +
    "<p>Einige junge Tiere nickten heftig.</p><p>Sie nickten immer, wenn der Ara sprach.</p>" +
    "<p>Seine Angst klang so prächtig wie seine Federn.</p>",

  "hai.p21": "<p>Später sprach eine sehr alte Eule mit langsamer sanfter Stimme.</p>" +
    "<p>„Ich erinnere mich an Kai,“ sagte sie.</p><p>„Das kleine Samenkorn, das mit Pip pflanzte.</p>" +
    "<p>Gütige Hände ändern sich nicht, wenn sie hoch werden.“</p><hr>" +
    "<p>Einige Tiere wandten sich ab,</p><p>weil die Eule nicht berühmt war.</p>" +
    "<p>Hai hörte den ganzen Stamm hinab zu.</p>",

  "hai.p22": "<p>Hai Ikthiss schrie sie nicht still.</p>" +
    "<p>Er schüttelte den Ara nicht vom Ast.</p><p>Er trat aus stillem Holz</p>" +
    "<p>und wurde ein gehender hoher Baum —</p><p>lange gütige Glieder, tiefe Wurzelfüße,</p>" +
    "<p>ein Gesicht, dem sie endlich begegnen konnten.</p><hr><p>Nicht um zu überragen.</p><p>Um nah zu kommen.</p>",

  "hai.p23": "<p>Einige Tiere traten zurück.</p><p>Pip nicht.</p><p>Pip erinnerte sich an die Melone</p>" +
    "<p>und den Grabstock</p><p>und die neuen Samen.</p><hr>" +
    "<p>Hai beugte sich wie eine Brücke aus Holz und Güte.</p><p>„Kommt,“ sagte er.</p>" +
    "<p>„Nicht unter mir.</p><p>Mit mir.“</p>",

  "hai.p24": "<p>Er hob Pip zuerst —</p><p>nicht um Macht zu zeigen,</p><p>sondern um den Blick zu teilen.</p>" +
    "<p>Dann hob er die Eule.</p><p>Dann die, die ihn gefürchtet hatten.</p>" +
    "<p>Sogar den Ara, als der Ara bereit war.</p><hr>" +
    "<p>Jeder Freund stieg zu Hais Höhe</p><p>und wurde mit ihm ein Riese —</p>" +
    "<p>dasselbe Gesicht, dasselbe Herz,</p><p>nur die Augen nun auf Höhe der fernen Hügel.</p>",

  "hai.p25": "<p>Aus der hohen stillen Luft</p><p>sahen sie das ganze Tal auf einmal —</p>" +
    "<p>den Dschungelteich, den alten Garten,</p><p>die Orte, wo Lob laut gewesen war,</p>" +
    "<p>und die Orte, wo Angst klein gewesen war.</p><hr><p>Hai fühlte Frieden.</p>" +
    "<p>Er brauchte nicht den höchsten Jubel.</p><p>Er brauchte nur wahre Freunde,</p>" +
    "<p>die so weit sehen können, wie Liebe sieht.</p><hr><p>Solange jemand frei sagen konnte</p>" +
    "<p>„ich hatte Angst“</p><p>und trotzdem willkommen gehoben wurde,</p>" +
    "<p>konnte der ferne Blick weitergehen</p><p>auf vielen starken Beinen.</p>",

  "hai.p26": "<p>Und so blieb Hai Ikthiss</p><p>ein lebendiger hoher Gefährte,</p>" +
    "<p>der mit einem leeren Becher Lob sitzen konnte,</p><p>der flickte, was von unten brach,</p>" +
    "<p>und der jeden willigen Freund</p><p>zu geteiltem fernem Blick hob.</p><hr>" +
    "<p>Das Tal fühlte sich weiter an,</p><p>weil er es tat.</p><hr>" +
    "<p>Nun konnten alle so weit sehen wie Hai.</p><hr>" +
    "<p>Er goss weiter sanft.</p><p>Und die Welt ließ ihn weiter wachsen.</p>",

  "hai.c1.title": "<h2>Kapitel Eins - Sein</h2><p><em>Hai und das leere Lob</em></p>",
  "hai.c2.title": "<h2>Kapitel Zwei - Flicken</h2><p><em>Kai geht unten</em></p>",
  "hai.c3.title": "<h2>Kapitel Drei - Sehen</h2><p><em>Hai und der ferne Blick</em></p>"
};
